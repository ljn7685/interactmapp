...
class Marketing extends Component {
    constructor (props) {
        ...
        this.isUsed = false; //判断是否已经再次取过pid
    }
     ...
     */
    navigatorCommonRule = (hasGet) => {
        const adRenderRule = navigatorConst[this.props.type];
        if (isEmpty(adRenderRule)) {
            return;
        }
        const pid = marketingPublic[adRenderRule.pidFunc](this.from, hasGet);
        if (!isEmpty(pid)) {
            // 如果有pid的话，冲冲冲
            this.openAd({
                pid,
                ...
                error_callback: (error) => {
                    if( !this.isUsed ){
                        //取到的pid广告没有上线 去取线上的默认广告 只取一次
                        this.navigatorCommonRule(true);
                        this.isUsed = true;
                    }
                }
            });
     ...
     * @Description 打开相应广告
     */
    openAd = ({ pid, state, type, error_callback = undefined }) => {
        triggerAdInfoByPid({
            pid,
            state,
            type,
    ...
    render = () => {
        ...
        let key = type === MARKETING_TYPE.modalVip ? MARKETING_TYPE.modalVip : `${location}_${currentPid}`;
        // let key = type === MARKETING_TYPE.modal ? : key;
