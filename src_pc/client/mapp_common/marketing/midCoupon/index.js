...
import { AD_TYPE, MARKETING_TYPE, MARKET_BEACON_CONST } from "tradePublic/marketing/constants";
...
class MidCoupon extends Component {
    ...
    componentDidMount(){
        ...
        if(isEmpty(ad) || !this.getCountTime(ad.adInfo.user_define.body.count_down)) {
            return null;
        }
        //广告-角标-展现埋点
        marketingBeacon(MARKET_BEACON_CONST.show, pid, 'promotion');
    }
    ...
    */
    clickCoupon = (content) => {
        ...
        //手机角标点击埋点
        marketingBeacon(MARKET_BEACON_CONST.click, this.props.pid, 'promotion');
        const { ad } = this.props;
        if(isIOS()) {
            goClick({
            ...
            //续费角标是否有弹窗
            if( !isEmpty(ad.adInfo.user_define.body.add) && ad.adInfo.user_define.body.add == '没有' ){
                goClick({
                    customType: AD_TYPE.FUWU_ORDER,
                    customUrl: ad.adInfo.dest_url,
                    adData: ad.adInfo,
                });
                afterFunc();
                this.closeModal();
