...
*/
export const triggerAdInfoByPid = ({ pid, state = AD_STATE.SHOULD_SHOW, type, callback = NOOP, error_callback = handleError }) => {
            ...
            switch(type) {
                ...
                case MARKETING_TYPE.modalVip: {
                    ...
                    Promise.all(marketingPromiseList).then((adData) => {
                        //实际展现的广告数据(有的pid下没有创意，导致会有空的情况展现出来)
                        let showAdData = [];
                        adData.map(item => {
                            if(item.pid == pid) {
                                if(storage.getItemSync('showAdPid') == 'true'){
                                    my.alert({
                                        title:'',
                                        content:'cid:' + JSON.stringify(item.adData.creative_id) + ', ' + 'pid:'+JSON.stringify(item.adData.pid)
                                    });
                                }
                            ...
                            if(!isEmpty(item.adData)){
                                showAdData.push(item);
                            }
                        });
                        let data = createAdData(type, showAdData, state, type);
                        data.currentPid = pid;
                        marketConsole('log', `≠≠≠${type}广告冲冲冲~≠≠≠ pidList:${pidList}`);
