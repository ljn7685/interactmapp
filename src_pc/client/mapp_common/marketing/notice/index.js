...
class Notice extends Component {
    ...
    componentDidMount(){
        ...
        if(!isEmpty(ad)) {
            //公告，弱提醒默认展开
            const noticeTextList = this.returnShowMessage(ad.adInfo);
            this.setState({
                noticeExpanded: true,
                currentNotice: noticeTextList[0],
                noticeTextList
            })
        }
    }
    ...
    */
    weakAdBeacon = (type) => {
        ...
        let level = `${getCurrentPageName()}_weak`;
    ...
    render () {
        ...
        const { noticeExpanded, currentNotice, current, noticeTextList } = this.state;
        // const textList = this.returnShowMessage(ad.adInfo);
        if(isEmpty(noticeTextList)) {
        } else  {
