...
import {hideLoading,showLoading} from "mapp_common/utils/loading";
...
export async function goLink  (url)  {
    ...
    if (isIOS()) {
        ...
        contactCustomerService(contactContent, null, 'ad');
        return;
    }
...
/*
 * @Description 跳转页面 千牛头条等跳转
*/
export const goPage = (url) => {
    const title = `假装跳转这个页面了~~ ${url}`;
    const isTrade = ENV.app === 'trade';
    ...
    if(isPC()) {
        ...
        switch(type) {
            ...
            case AD_TYPE.CONTACT_KEFU: {
                ...
                contactCustomerService(service, null, 'ad');
                break;
            }
            // 2: 订购插件类型
            ...
            default: {
                ...
                if (isIOS()) {
                    ...
                    if(type === MARKETING_TYPE.notice) {
                        ...
                        contactCustomerService(text, null, 'ad');
                    }
                } else {
                    // 非ios用户直接跳转服务市场
                    goLink(url);
...
const beforePayBeacon = (adData) => {
    ...
    const { pid, cname, cid, payUrl, creative_name, creative_id, modalVipPid } = adData;
    const beaconObj = {
        p: payBeaconParent,
        e: payBeacon,
        m1: modalVipPid || pid,
        m2: creative_name || cname,
        m3: creative_id || cid,
