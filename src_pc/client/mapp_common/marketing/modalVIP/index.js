...
import { getCurrentPageName, hideTabBar, isEmpty, isIOS, showTabBar, TAB_BAR_PAGES, navigateTo } from "mapp_common/utils";
...
import { isNotVip } from "mapp_common/utils/userInfo";
@connect((store) => {
...
class ModalVIP extends Component {
    ...
    */
    renderBtns = (current, ad) => {
        ...
        return [2, 3, 4].map(item => {
            ...
            if (!isEmpty(body[`btn-type${item}`]) && body[`btn-type${item}`] === '1' && isNotVip()) {
                isAutoRenew = true;
            }
            return (
    ...
    */
    enterAutoPayIntro = (linkUrl) => {
        const projectCode = ENV.app == 'item' ? 'item_auto_renew' : 'trade_auto_renew';
        api({
            apiName: 'aiyong.user.autorenew.status.get',
            domain: ENV.hosts[ENV.app],
    ...
    */
    clickAutoPayBtn = (isAuto) => {
        ...
        if(isAuto) {
            ...
            this.closeAD(1);
            navigateTo({
                url: `/public/mapp_common/marketing/autoRenew/index`,
                params: {
                    url:  this.pendingLink,
                    ad: this.ad,
                },
            });
        } else {
