...
export function api (
    ...
    if (process.env.TARO_ENV === 'h5') {
        ...
        if (apiName) {
            let newArgs = {...args };
            //如果是多店切换到其他店铺，加上这两个参数， 只在订单管理和打单发货界面。
            if (ENV.device === 'pc') {
                if (currentShopStatus().isOtherShop && ["tradeManagement", "batchPrint"].includes(getCurrentPageName())) {
                    newArgs = { ...args, 'targetNick': currentShopStatus().currentShopName, 'targetStoreId': 'TAO' }
                }
            }
            applicationApi({
                args: newArgs,
...
export function applicationApi ({ args, apiName, path, method = 'POST', headers = {}, version = 1, callback, errCallback }) {
    phpSessionIdDeferred.then(() => {
        ...
        getCloud().application.httpRequest(data).then(res => {
            ...
            if(apiName === 'aiyong.item.autoadjust.exclude.numiid.cid.get' && res.indexOf('null') !== -1) {
                data = res.replace('null', '');
            }
            if (res === 'success') {
                callback(res);
                return;
