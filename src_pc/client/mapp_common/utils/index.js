...
import { showErrorDialog } from "tradePolyfills/index";
...
export function navigateBack (delta = 1, success) {
    Taro.navigateBack({ delta, success });
}
...
export function getCurrentPageName  ()  {
    ...
    if (ENV.device === 'pc') {
        return page[page.length - 1].data.currentMain.substr(1);
    }
    ...
    return new Promise(resolve => {
        my.getServerTime({
            ...
            fail: (err) => {
                errCallback();
                resolve();
                showErrorDialog('温馨提示', '获取系统时间出错！', JSON.stringify(err));
            },
        });
    });
}
