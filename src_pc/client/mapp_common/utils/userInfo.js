...
import { newUserModel } from "mapp_common/marketing/newUserModel";
 ...
 * 获取用户降级信息
 * @Author ZW
 * @date   2020-05-22T21:48:45+0800
 * @return {Object}                 降级信息对象
 */
const getDownGradeInfo = () => {
    api({
        apiName: 'aiyong.marketing.settings.fixedad.get',
        method: '/iytrade2/getFixedAdSettings',
        args: { product: `qn_${app}` },
        callback: (rsp) => {
            if (rsp && rsp.downGrade && rsp.downGrade.content) {
                getSettings().downGradeInfo = JSON.parse(rsp.downGrade.content);
            } else {
                Logger.error('getFixedAdSettings failed', rsp);
            }
        },
    });
};
...
export function authLimited (authResult) {
    ...
    try{
        ...
        Taro.showToast({ icon: 'fail', title: JSON.stringify(formatError(e)) });
    }
    let showConfirmModalJson = {
        content:"",
        openChatText:"",
    };
    if(app == 'trade') {
        showConfirmModalJson.content = '爱用交易授权即将过期，自动评价、差评拦截、自动合单功能将无法正常使用，请立即联系主账号进行授权';
        showConfirmModalJson.openChatText = '爱用交易授权即将过期，现在需要使用主账号的登录千牛，打开爱用交易授权，如不授权，所有账号将无法进入爱用交易，自动评价、差评拦截、自动合单功能无法正常使用，请立即授权爱用交易。';
    }else{
        showConfirmModalJson.content = '爱用商品授权即将过期，违规词检测、批量修改、手机详情功能将无法正常使用，请立即联系主账号进行授权';
        showConfirmModalJson.openChatText = '爱用商品授权即将过期，现在需要使用主账号的登录千牛，打开爱用商品授权，如不授权，所有账号将无法进入爱用商品，违规词检测、批量修改、手机详情功能无法正常使用，请立即授权爱用商品。';
    }
    showConfirmModal({
        title:'温馨提示',
        content: showConfirmModalJson.content + unAuthorizedPermissionsStr,
        showCancel: false,
        confirmText: '联系主账号授权',
        onConfirm:() => {
            openChat({
                nick: getUserInfo().userNick,
                text: showConfirmModalJson.openChatText,
            });
        },
    });
}
...
export const fetchUserInfoFromTcUser = ({ callback, nick  }) => {
    ...
    api({
        ...
        callback:res => {
            ...
            callback(newUserInfo);
            if (res.needauth == '1' && ENV.app == 'trade') {
